...
func PeriodicConsumption(lineId uint) (string, map[string]string, error) {
	fmt.Println("ENTRANDO")
	j, err := loadJob(lineId)
	if err != nil {
		params := make(map[string]string)
		params["lineId"] = string(lineId)
		return "Line {lineId} does not exist", params, nil
	}

	if j.ContinueRunning {
		// HACER EL TRABAJO
		res, mapRes, err := GetConsumption(lineId)
		if err != nil {
			j.scheduleNextRun(mapRes["consumptionHash"])
		}
		updateJob(j)
		return res, mapRes, err

	} else {
		params := make(map[string]string)
		params["lineId"] = string(j.LineId)
		return "Periodic task for line {lineId} has been canceled", params, nil
	}
}
